...
def load_data(path="./celeba-dataset/img_align_celeba/img_align_celeba/", n_samples=1000, im_ref_size=256):
    ...
    assert (file_exists,
            "dataset does not exist: Please download the celeba dataset under this directory, and change the root directory name as celeba-dataset")
    ...
    X = np.array([np.array(
        cv2.resize(cv2.copyMakeBorder(cv2.imread(path + f), w - 1, w + 1, h - 1, h + 1, cv2.BORDER_REPLICATE), dim)) for
                  f in files])
    X = X.astype('float32')
    X = (X - 127.5) / 127.5
    return X
